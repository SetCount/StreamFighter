...
function matchAllPresets(p: StartggPlayer, presets: PlayerPreset[]): PlayerPreset[] {
    ...
    for (const x of presets) {
        if (x.name.toLowerCase() === lc || x.aliases?.some(a => a.toLowerCase() === lc)) {
            matches.push(x);
        }
    }
    return matches;
...
function matchPreset(p: StartggPlayer, presets: PlayerPreset[], overrides?: Map<number, PlayerPreset>): PlayerPreset | undefined {
    if (overrides && p.id) {
...
export function collectAmbiguities(set: StartggSet, presets: PlayerPreset[]): Ambiguity[] {
...
function playerFromStartgg(p: StartggPlayer, presets: PlayerPreset[], overrides?: Map<number, PlayerPreset>): Player {
