...
function formatKeyCombo(e: KeyboardEvent): string {
    ...
    const key = e.code;
    const isModifierOnly = ['ControlLeft', 'ControlRight', 'AltLeft', 'AltRight',
...
function friendlyKeyName(code: string): string {
    ...
    const map: Record<string, string> = {
        ...
        Escape: 'Esc', Delete: 'Del', Insert: 'Ins', Home: 'Home', End: 'End',
...
export default function HotkeysEditor({ value, onChange, onCommit }: Props) {
    ...
    const groups = HOTKEY_ACTIONS.reduce<Record<string, HotkeyAction[]>>((acc, a) => {
        (acc[a.group] ??= []).push(a);
        return acc;
    }, {});
    ...
    const conflict = (actionId: string): string | undefined => {
        ...
        const other = HOTKEY_ACTIONS.find(a => a.id !== actionId && value.bindings[a.id] === combo);
        return other?.label;
    };
                                    ...
                                    <div key={action.id} className="hotkey-row">
                                        ...
                                        <div className="hotkey-binding-group">
                                            <button
                                                ...
                                                title={isRecording ? 'Press any key combo…' : 'Click to rebind'}
                                            >
                                                {isRecording
                                                    ? 'Press a key…'
                                                    : bound
